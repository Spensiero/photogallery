...
export default class Gallery extends Component<any,State> {
    ...
    setGalleryContent(){
        ...
        for (let item of  this.state.dataImages) {
            ...
            arrPictures.push(<li key={key}><Link to={"/detail?"+ data}><img src={item.data.thumbnail}  alt="image"/></Link></li>) 
        }
    ...
    render() {
        return (
            <div style={{height:  this.state.scroll ? '100%' : window.innerHeight, overflowY: this.state.scroll ? 'inherit' : 'hidden'}}>
                <div>
                    <input role="search" id='search' type='text' placeholder='Search' onChange={(e)=> this.filter(e)}/>
